...
public struct TrafficView: View {
    ...
    private func heatmapCell(hour: Int) -> some View {
        ...
        let totalBytesInHour = pointsInHour.reduce(UInt64(0)) { $0 + ($1.downloadBytes + $1.uploadBytes) }

        let intensity: Double
        if totalBytesInHour > 0 {
            let maxRecorded = viewModel.trafficDataPoints.map { $0.downloadBytes + $0.uploadBytes }.max() ?? UInt64(10_000_000)
            let ratio = Double(totalBytesInHour) / Double(max(maxRecorded, UInt64(1_000_000)))
